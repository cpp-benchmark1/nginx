...
// Example 1: Heap overflow using recvfrom()
void vulnerable_heap_overflow1(int socket_fd) {
    ...
    // SOURCE: Reading user input from socket using recvfrom
    ssize_t bytes_received = recvfrom(socket_fd, buffer, 100, 0, 
...
4. Inside the container, compile the code:
   gcc -o vuln-cwe122-ex1 plant-vuln-cwe122-ex1.c
...
- The strcpy will overflow the destination buffer
