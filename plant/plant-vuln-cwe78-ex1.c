...
void handle_command(int socket_fd, const char* client_ip) {
    ...
    // SOURCE: Vulnerable to command injection - receiving untrusted input from socket using recv
    ...
    if (send(socket_fd, output, strlen(output), 0) < 0) {
...
/*
To test:
1. Compile: gcc -o vuln-cwe78-ex1 plant-vuln-cwe78-ex1.c
2. Run: ./vuln-cwe78-ex1
